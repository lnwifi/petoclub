...
interface DestacarPetModalProps {
  visible: boolean;
  petId: string;
  onClose: () => void;
}

export default function DestacarPetModal({ visible, petId, onClose }: DestacarPetModalProps) {
  ...
  // Consulta el estado de la mascota en Supabase
  ...
  const handlePayment = async () => {
    if (!selectedPlan) {
      Alert.alert('Selecciona un plan', 'Por favor selecciona un plan para destacar tu mascota.');
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      if (res.ok && data.init_point) {
        ...
        Alert.alert('¡Listo!', 'Cuando se confirme el pago, tu mascota será destacada automáticamente.');
      } else {
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={styles.overlay}>
        <View style={styles.container}>
          <MaterialIcons name="pets" size={60} color="#fbaa30" style={{marginBottom: 18}} />
          <Text style={styles.title}>Destaca tu mascota</Text>
          <Text style={styles.description}>
            Elige uno de los planes para destacar tu mascota y aumentar su visibilidad en la app.
          </Text>
          ...
          {isFeatured && planEndDate && (
            <View style={{marginBottom:18, alignItems:'center'}}>
              <Text style={{color:'#388e3c', fontWeight:'bold', fontSize:16, marginBottom:2}}>¡Tu mascota está destacada!</Text>
          ...
          <TouchableOpacity
            ...
            disabled={loading || waitingPayment || isFeatured}
          >
            {loading ? <ActivityIndicator color="#fff" /> : waitingPayment ? <Text style={styles.buttonText}>Esperando confirmación...</Text> : isFeatured ? <Text style={styles.buttonText}>¡Ya eres destacado!</Text> : <Text style={styles.buttonText}>Destacar mi mascota</Text>}
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 26, fontWeight: 'bold', color: '#fbaa30', marginBottom: 14, textAlign: 'center' },
  ...
  planCard: { backgroundColor: '#fff', borderWidth: 2, borderColor: '#ffe0b2', borderRadius: 18, paddingVertical: 22, paddingHorizontal: 22, alignItems: 'center', flex: 1, marginHorizontal: 2, shadowColor: '#fbaa30', shadowOpacity: 0.08, shadowRadius: 8, elevation: 2, minWidth: 100 },
  planCardSelected: { borderColor: '#fbaa30', backgroundColor: '#fffbe6', elevation: 6, shadowOpacity: 0.22 },
  planPriceBig: { fontSize: 30, color: '#fbaa30', fontWeight: 'bold', marginBottom: 2 },
  ...
  button: { backgroundColor: '#fbaa30', paddingVertical: 14, paddingHorizontal: 38, borderRadius: 25, marginTop: 10, shadowColor: '#fbaa30', shadowOpacity: 0.18, shadowRadius: 9, elevation: 3 },
