...
CREATE OR REPLACE FUNCTION public.get_pending_matches_fixed(user_pet_id UUID)
RETURNS TABLE (
    id UUID,
    pet_id_1 UUID,
    pet_id_2 UUID,
...
GRANT EXECUTE ON FUNCTION public.get_pending_matches_fixed TO authenticated;

-- Drop old function
DROP FUNCTION IF EXISTS public.get_pending_matches;

-- Rename new function to old name
ALTER FUNCTION public.get_pending_matches_fixed RENAME TO get_pending_matches;
